...
int glx_swap(struct rb_video_glx *glx,const void *fb) {
  ...
  for (;i-->0;src++,dst+=3) {
    ...
    // This is what I would assume for red, copy the two provided bits four times:
    //dst[0]=(*src)&0x03; dst[0]|=dst[0]<<2; dst[0]|=dst[0]<<4;
    // but this seems truer to the TinyArcade, repeat the LSB, then copy as if it were 3-bit:
    dst[0]=(*src)&0x03; dst[0]<<=6; if (dst[0]&0x40) dst[0]|=0x20; dst[0]|=dst[0]>>3; dst[0]|=dst[0]>>6;
