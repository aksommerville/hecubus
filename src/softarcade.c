...
void softarcade_fill_rect(
  ...
  if (x>dst->w-w) {
    if ((w=dst->w-x)<1) return;
  }
  if (y>dst->h-h) {
    if ((h=dst->h-y)<1) return;
  }
  uint8_t *row=dst->v+y*dst->stride+x;
...
int16_t softarcade_font_render(
  struct softarcade_image *dst,
  int8_t dstx,int8_t dsty,
...
int16_t softarcade_font_measure(
  const struct softarcade_font *font,
  const char *src,int8_t srcc
) {
  int16_t rtn=0;
